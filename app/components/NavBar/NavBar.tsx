import Link from "next/link";
import React from "react";
import SearchResult from "./SearchResult";
import Image from "next/image";

export default function NavBar() {
  return (
  <div className="w-full flex text-blue-700 md:px-[20%] fixed bg-white md:justify-between items-start md:items-center justify-start  shadow-md flex-col md:flex-row">
      {/* <Link href="/" className="logo text-xl text-gray-900 font-bold"></Link> */}
      <ul className="flex gap-6 w-full items-center font-semibold ">
        <li><Link href="/"  className="flex items-center">
        <Image src="/logo.png" alt="Logo Maxima" width={70} height={70} className="inline-block mr-2" />
        <div className="name flex flex-col ">
          <span className="text-lg font-bold italic text-red-500">Bintang Survey</span>
          <span className=" text-gray-600 text-xs">Survey Instrument&apos;s</span>
        </div>
        </Link></li>
      </ul>
      
      {/* <SearchResult /> */}
    </div>);
}
